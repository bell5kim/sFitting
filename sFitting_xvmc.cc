...
void SpectCalc(float x, float a[], float *y, float dyda[], int na)
{
    ...
    *y      = P(z, L, B, Emax, Emin, N);
    dyda[1] = dPdL(z, L, B, Emax, Emin, N);
    dyda[2] = dPdB(z, L, B, Emax, Emin, N);
    dyda[3] = dPdEmin(z, L, B, Emax, Emin, N);
    dyda[4] = dPdEmax(z, L, B, Emax, Emin, N);
    dyda[5] = dPdN(z, L, B, Emax, Emin, N);
}
...
int main(int argc, char *argv[])
{
    ...
    const long MA  = 5;             // number of fit parameters
    ...
    printf ("# initial Emin = %f ---> final Emin = %f\n", Emin, a[3]);
    printf ("# initial Emax = %f ---> final Emax = %f\n", Emax, a[4]);
    float NORM = 1.850;
    if (a[5]-10.0 < NORM || a[5]-10.0 > 1.5) NORM = a[5]-10.0;
    ...
    for (int i=0; i<nData; i++)
    {
        Dc[i] = P(z[i],a[1],a[2],a[4],a[3],a[5]);
        if(Dc[i] >= peakPc)
        {
            peakPc = Dc[i];
            peakEc = z[i];
